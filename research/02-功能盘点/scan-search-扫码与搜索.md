...
| F-11 桌面端预览面板 | 选中结果拉详情，显示"临床提示/安全确认" | 部分实现（内容造假，桌面冻结） | 删除 | P2 |
| F-12 最近搜索 / 分类快捷 | 空查询时的历史关键词与分类导航 | 假实现（永远空列表，无数据源） | 删除 | P2 |
...
- 结论：改造。
- 改造方案：链路保留；进 AI 路径前加登录门控（复用 `pushAuthRequiredRoute`）；后端 async 端点要么客户端接入（大图识别确实更适合异步+轮询），要么从 Lucent 删掉，不留双轨。
- 优先级：P1。
...
- 结论：改造。
- 改造方案：删掉面向用户的"置信度 %"展示——改为可解释的来源标签（已有："批准文号匹配/名称模糊匹配"，`recognize_dialog.dart:306-313`），符合 Vision"证据可追溯、不笼统"的要求；"确认"主按钮改为"加入药箱"（复用 F-9 链路），top 结果改为排序后首位。
- 优先级：P0。
...
- 结论：删除。
- 改造方案：桌面冻结不做功能对等，预览面板连同 `MedicineSearchSafetyPreview`、`fetchDetail` 一起删；移动端真正需要的是药品详情页（见模块级缺口），届时详情数据直接展示说明书原文（后端 `getDetail` 已返回完整字段：适应症/用法用量/禁忌/不良反应/相互作用等，`Lucent/.../adapters/cn.service.ts:64-86`、`drugbank.service.ts:66-90`），不需要"预览"这种二手摘要。
- 优先级：P2。
...
- 结论：删除。
- 改造方案：删掉两个 section 组件、`MedicineSearchDashboard`/`MedicineSearchCategory` 实体与相关 l10n 键。若未来要做搜索历史，用本地存储真实记录后再加回；分类导航与"关键词搜索"形态重复，不建议复活。
- 优先级：P2。
...
**冗余**：桌面预览面板（F-11）内容造假应删；最近搜索/分类（F-12）与 `MedicineSearchDashboard` 实体是死脚手架应删；后端 `recognize/async` 队列端点无消费方，应接入或删除。
