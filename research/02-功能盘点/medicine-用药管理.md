...
- 结论：改造/归档。接口与 provider 代码及注释保留，标注不接入主路径（暂无 UI 消费方）。若未来要"随机安全贴士"，应在药品详情页内以审核内容卡片形式重做。
- 优先级：P2。
...
- 实现真实性：死代码。
- 结论：改造/归档。字段与其拷贝（`promisePoint*` 4 个 MedicineCopyKey 及 l10n 键）保留并注释标注"不接入主路径"，避免后续误用；其中 `alerts` 恒空问题并入"用药安全摘要卡"改造（从后端风险检查结果实时聚合，见 F-13），不再按死字段清理。
- 优先级：P2。
...
- 实现真实性：死代码（防御性枚举值）。
- 结论：改造/归档。枚举保留为历史兼容（漏服状态由 `overdueUnconfirmed` 计算派生、不落库为 missed），代码与注释保留、标注不接入主路径；若未来要落"漏服"标记，应在后端 collector 侧产出，前端保持消费方即可。
- 优先级：P2。
