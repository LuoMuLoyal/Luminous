...
| F-9 提醒投递历史（只读） | 详情页展示投递审计行，数据来自真实 userReminderDelivery | 部分实现 | 保留（待 worker 补全） | P2 |
...
| F-13 用药安全摘要卡（主页） | 风险等级/指标/告警芯片，数据全部来自后端检查记录 | 真实现 | 保留 | P0 |
| F-14 药品详情与说明书 | 移动端无任何药品详情页，cn/drugbank 详情数据无消费出口 | 功能缺失 | 新建 | P1 |
| F-15 快捷操作区（4 入口） | 导航到添加/搜索/提醒/风险检查 | 真实现 | 保留 | P2 |
| F-16 safety tips（后端 API+客户端 provider） | 随机安全提示接口，客户端 provider 无任何 UI 消费方 | 死代码 | 删除 | P2 |
| F-17 workspace 死字段 | promisePoints/alerts/metricDosesToday 从不渲染 | 死代码 | 删除 | P2 |
| F-18 DoseLogStatus.missed 死枚举 | 前后端均无写入方，仅 UI switch 分支残留 | 死代码 | 删除 | P2 |
| F-19 处方导入入口 | 仅 Toast 提示延后，无功能 | 假实现（诚实占位） | 保留占位 | P2 |
...
- 实现真实性：部分实现。数据是真的，但语义与后端合同脱节，未来会出现"主页 33% 与 Report/Today 的 1/3 已确认、1 超时未确认"
  的口径打架。
- 结论：改造。接后端 `ObservedMetric`（value/state/coverage）或至少改用与 Today collector 一致的槽位统计
  （分母只计"已到期"槽位，未到期不数）。
- 改造方案：P1 前端 mapper 层把今日依从率改为"已确认 / 已到期槽位"，未到期与无覆盖显示 `--`；
  P2 后端在 dose-logs 或 workspace 接口暴露当日 slot 统计对象，消除两端口径漂移。
- 优先级：P1。
...
- 实现真实性：真实现，全部字段来自后端记录（`medicineRiskCheckLastUpdated/Stale` 如实显示时间与过期态）。
- 结论：保留。
- 优先级：P0。
...
- 现状：客户端没有任何移动端药品详情页。搜索/扫码结果在移动端无 tap 跳转（`search/.../results.dart` 仅桌面
  预览面板包 FTappable，而该面板内容造假已被 scan-search F-11 判删）；`/medicine/reminders/:medicineId`
...
- 结论：新建。移动端药品详情页复用现有 `GET /medicines/:id?source=`（含缓存），展示说明书字段 +
  加入药箱按钮 + 风险相关字段入口；可与 Reminder 详情页的药品卡合并跳转。桌面预览面板删或复用同一页面数据。
...
- 结论：删除。若未来要"随机安全贴士"，应在药品详情页内以审核内容卡片形式重做，而不是先留 provider。
- 优先级：P2。
...
- 实现真实性：死代码。
- 结论：删除字段与其拷贝（`promisePoint*` 4 个 MedicineCopyKey 及 l10n 键一并清理），避免后续误用。
- 优先级：P2。
...
- 实现真实性：死代码（防御性枚举值）。
- 结论：删除（或保留枚举但删 UI 分支——建议两端一并删，真需要时再加）。
- 优先级：P2。
...
- 结论：保留占位（首阶段人群不依赖处方导入，扫码+搜索已覆盖大部分场景）；若 3 个月内无排期可删入口。
- 优先级：P2。
...
- 结论：保留。短信对目标人群（短期事件、自行买药）无必要，不要投入。
- 优先级：P3。
...
- 实现真实性：真实现。
- 结论：保留。`today-今日建议.md` 已指出 `skip_dose` 死参数应删或接真，此处从用药侧确认：主页“跳过”按钮走 mark(skipped)
  是真实动作，与建议卡无冲突。
- 优先级：P0。
...
- **safety-tips / 投递历史 worker / SMS**：safety-tips 接口属于"做了没用"的投入（F-16 判删）；投递历史 worker
...
- **价值判断**：用药管理是 Luminous 当前数据链最完整、风险边界最明确的平级健康领域——档案（currentMedicines）→
  ...
  审计结论：21 个功能点中真实现 14 项、部分实现 3 项、死代码/缺失 4 项，无系统性造假。
- **整体改造建议（按优先级）**：
  1. P0 保留主线不动；清理 F-16/F-17/F-18 三处死代码（低风险、立即做）。
  ...
  3. P2 主页打卡加撤销（F-3）；提醒组保存半失败回滚（F-6）；投递历史补本地/push 渠道记录（F-9）；
     risk-check 客户端 unknown 枚举兜底去误导（F-11）；处方占位按排期删留（F-19）。
- **一句话**：本模块是健康伙伴中可信度要求最高的平级领域，方向正确、实现诚实，剩余工作不是返工而是"把已有数据资产变成用户
  看得见的页面"（详情页）与"口径统一"（依从性），以及清掉少量残留死代码。
