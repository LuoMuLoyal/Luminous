...
void main() {
  ...
  group('SyncWorker.flush', () {
    ...
    test(
      'parks a non-retryable DioException instead of spending attempts',
      () async {
        ...
        when(() => mockDao.fetchReady()).thenAnswer((_) async => [entry]);
        when(
          () => mockDao.markSyncing('rejected-001'),
        ).thenAnswer((_) async {});
        when(
          ...
          throw DioException(
            ...
            response: Response<dynamic>(
              ...
              data: const {
                ...
                'detail':
                    'The request does not identify which dose log it '
                    'refers to.',
                'retryable': false,
              },
            ),
    ...
    test(
      'keeps retrying when the server declares the failure retryable',
      () async {
        final entry = _createEntry(
          id: 'retryable-001',
          retryCount: 1,
        ...
        when(() => mockDao.fetchReady()).thenAnswer((_) async => [entry]);
        when(
          () => mockDao.markSyncing('retryable-001'),
        ).thenAnswer((_) async {});
        when(
          () => mockDao.markFailed(
            'retryable-001',
