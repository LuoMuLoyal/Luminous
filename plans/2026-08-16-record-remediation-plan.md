...
3. 建设 vital 时间序列基建:体重(手动+平台导入)、血糖趋势、血压合并为统一基建(§2.6 已决策,速览表无落位但必须列入)。
4. 降低餐食分析链路成本:vision 识别分层触发(§1 改造建议)。
5. 桌面资产(趋势图、月历标记)改造为真实数据,形态挂起待 ADR-0012。
6. 落实 Today 联动方向:饮食/饮水/睡眠/心情升级为主动建议平级原料(门控阈值未定,见「六、不确定点」)。
...
#### P1-1 Record 页摘要网格接线 + 饮水 ml 角标恢复(调研 §4.1)
...
#### P1-2 详情页饮水进度卡目标值改读 user-settings(调研 §4.4,本文档写全)
...
- 方案:饮水目标改读 `user-settings.waterTargetCount`,按 `waterTargetCount × 单次默认量` 换算为 ml 展示;移除 `_waterDailyTargetMl` 常量。契约来源二选一(经 `user-settings` API 直连,或复用 §4.1 接线后的 summary 契约)未定,见「六、不确定点」——默认优先直连 `user-settings` API,与 Today Analysis 同源。
...
- 依赖:无;但「候选洞察确有需要」的触发判定机制未定,见「六、不确定点」。

#### P1-4 vital 时间序列基建(体重/血糖/血压统一)(调研 §2.6,已决策)
...
- 方案:
  - 档案字段 `weightKg` 保留为当前基线;新增体重时间序列记录,来源 = 手动录入 + 平台导入(导入来源未定,见「六、不确定点」)。
...
#### P2-1 分析确认入口简化(调研 §1.7)

- 现状:确认分析结果需进入编辑页勾选「确认当前结果」。
- 方案:详情页摘要卡(`analysis_summary_card.dart`)直接提供「确认」按钮,走现有 PATCH `analysisStatus='confirmed'` 链路(`buildConfirmedMealPayload` + 模板学习不变)。
- 分工:纯客户端;后端不动。

#### P2-2 静态残留清理(调研 §4.1 P2)

- 现状:`lucent.dart` 中 `_staticSummary`/`_staticWeekDays` 仍有静态残留。
- 方案:P1-1 接线完成后,移除 `_staticSummary`;核对 `_staticWeekDays` 的实际引用范围(正文未展开,见「六、不确定点」)后一并清理。
- 依赖:P1-1。

#### P2-3 桌面趋势图真实数据化(调研 §4.2,挂起待 ADR-0012)
...
#### P2-4 桌面月历服务端标记(调研 §4.3,挂起待 ADR-0012)
...
#### P2-5 Today 联动:饮食/饮水/睡眠/心情升级为建议平级原料(调研 §5)
...
- 依赖:today 计划(第 4 位)的今日建议主线改造;门控阈值全部未定,见「六、不确定点」——阈值未定前本项不启动实现。
...
- **桌面/Web 形态挂起**:P2-3、P2-4 的形态决策统一引用 [`2026-08-14-product-surface-route.md`](2026-08-14-product-surface-route.md)(ADR-0012 待决策),本文不展开。
...
1. P1-1 摘要网格接线(无依赖,恢复既有能力)
2. P1-2 饮水目标读 user-settings(可与 P1-1 并行)
3. P1-4 vital 时间序列基建(被后续计划引用,尽量前置)
4. P1-3 餐食分析分层触发(触发判定机制澄清后启动)
5. P2-1 确认入口简化、P2-2 静态残留清理(P1-1 之后)
6. P2-3、P2-4 桌面资产(待 ADR-0012)
7. P2-5 Today 联动门控(待阈值决策 + today 计划落地)

## 六、不确定点(待决策)

- **P1-3 分层分析触发条件**:「候选洞察确有需要」的判定机制、由谁触发(客户端还是后端 worker)未定。
- **P2-3 / P2-4 桌面资产**:均「等待桌面/Web 独立调研后决定形态/后启用」,挂起状态,无方案细节、无时间点(ADR-0012)。
- **P1-2 饮水目标契约路径**:「经 `user-settings` API 或 summary 契约」二选一未定(默认倾向直连 `user-settings` API)。
- **P2-2 `_staticWeekDays` 残留范围**:正文其他处未展开,清理范围不完全清晰,实施前先核对引用。
- **P2-5 Today 联动门控阈值**:覆盖率、时间范围、变化幅度、可行动性的具体标准全部未定;由哪端实现门控也未定——阈值未定前不启动实现。
- **P1-4 体重时间序列**:「平台导入」指哪些平台/来源未说明;与 vital 基建的落地顺序、进入「纵向洞察」的具体形式未展开。
- **优先级落位**:§2.6 vital 基建与 §5 Today 联动在调研文档中无 P 级标注,本计划分别按「被后续计划引用」「阈值未定」列为 P1 / P2,可按决策结果调整。
