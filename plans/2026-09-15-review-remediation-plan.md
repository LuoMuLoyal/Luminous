...
### 3-1. 缓存反序列化硬 cast 容错【09-13 S-5】P2·S
...
### 3-2. quantity_sheet clamp 不回写 + late final 隐患【09-13 W-5 + 09-14 S-3 同处】P2·S
...
### 3-9. `weightInLb` 定位澄清【09-13 S-13】P2·S
...
### 3-10. auth 页骨架收敛 AuthScrollBody【09-14 "重复造轮子"修正案】P2·S
...
### 3-11. 生成模型 `ReportSummaryAsync/Stream*` 清理【09-08 W-2】P2·M（跨仓，合同级）
...
- **Wave 2（P2·A）**：3-1 → 3-2 → 3-3 → 3-4 → 3-5 → 3-6 → 3-7 → 3-8 → 3-9 → 3-10；3-11（合同级）与 §3-9/§4-19/§4-20 按 09-08 建议合批：同一测试文件（account_settings_page_test 改名 + key 去重 + 断言补强）一批做。
...
- **明确不做**（除非另行立项）：avatar 真草稿化重构（09-13 W-2 后半）、450ms debounce 注入钩子、cooldown 真时钟测试改造（§5-4 第三类）、card/scrollable 参数拆分、tab_branch_container late 改写、authGuarded 新 lint 规则、pre-push 回滚加 test、reviewModels 手工删模型（走 3-11 合同级路径）、§5-6 的静默 catch 追改。
