...
### P0 —— 文案/行为硬伤与死交互（约 20 项，建议 1-2 个迭代修完）

修复"明显错误"类问题，投入小、用户感知强：

1. 今日：快捷操作 3 条错误路由（3.1.7）；睡眠单位"7.5h 小时"（3.1.5）；"未记录 小时"fallback。
2. 记录：桌面月历写死"2025年5月"（3.2.1）；编辑页删除确认标题"解绑身份"+删除成功 toast"已保存"+删除后回退落点（3.2.4）；快捷弹层英文单位（3.2.5）。
3. 用药：铃铛红点常显+落点错（3.3.1）；"按时服用率"detail 错配（3.3.1）；风险三级 warning→destructive 同色（3.3.3）。
4. 报告：AI 硬编码中文占位/错误（3.4.4）；"预览评分"对正式用户（3.4.2）；状态徽章"未开通"误用（3.4.4）；移动端补医疗免责声明（3.4.4）。
5. 我的：身高解析丢数据 bug（3.5.3）；必填校验"请先填写验证码"（3.5.4）；三个表单枚举英文直出（3.5.4）；"去登录"红色误导（3.5.2+3.6.1）。
