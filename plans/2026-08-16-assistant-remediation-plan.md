...
- 依赖:F-3 的流式重发管线(见不确定点:F-3 断流补偿方案未定,两项共用同一路径)。
...
- 改造方案:
  - 二选一修复(未定,见不确定点):a) 后端 `confirm` 端点改为真实写入(服务端原子化);b) 客户端写入失败时反向调用 `reject` 补偿接口把线程置为失败态。按调研口径:只在可靠性与补偿测试证明需要时调整执行位置,不为展示复杂度加层。
...
- 改造方案:
  - 改摘要压缩:记忆块不灌原文,改为"会话标题 + 要点摘要",复用后端 `chatCompression` role 已有配置位(摘要 prompt、压缩粒度未定,见不确定点)。
...
- 改造方案:
  - 对 135 万条语料增加"可验证性"分层:优先只把有结构化来源(说明书/中国食物成分表/已审校数据)的 chunk 标记为可引用,医疗问答仅作兜底;检索 top-k 减半(现 `ASSISTANT_VECTOR_DEFAULT_LIMIT` 偏高;识别机制与具体数值未定,见不确定点)。
...
- F-3 断流补偿:开启有限重连(`maxReconnects: 2`)或断流时"继续生成"按钮复用 `retryLastMessage`(二选一未定,见不确定点);流式中断时 `streamingDraft` 残句保留为失败消息内容供复制,不再清空消失。
...
- **桌面/Web 形态**:assistant 页抽屉已区分移动端不透明抽屉/桌面 320px;桌面形态整体挂起项统一见 [`2026-08-14-product-surface-route.md`](2026-08-14-product-surface-route.md)(ADR-0012 待决策),本文不展开。
...
3. F-11/F-16 一致性裂缝修复 + F-9 记忆摘要压缩(P1 第一梯队)。
4. F-2 会话重命名/删除落地(后端端点先行,前端接线;顺带 F-1 小项)。
5. F-5b regenerate/resend 接线 + F-3 断流补偿(共用流式重发管线,一并验证)。
6. F-15 语料可验证性分层。
7. P2 批次:F-4 Markdown 模板 → F-17/F-18 归档标记 → 其余 P2 子项。

## 六、不确定点(待决策)

- **F-11 写入位置二选一未定**:a) 后端 `confirm` 端点真实写入(服务端原子化) vs b) 客户端失败时反向 `reject` 补偿接口——调研只给选项未拍板,且留有"只在可靠性与补偿测试证明需要时调整"的余地。
- **F-3 断流补偿二选一**:开有限重连(`maxReconnects: 2`) vs 断流时"继续生成"按钮,未定。
- **F-9 摘要压缩具体方案**:仅说"用 `chatCompression` role 已有配置位",摘要 prompt、压缩粒度、触发时机均未定义。
- **F-5 regenerate/resend 接口形态**:计划未单独成文(TODO.md 只有一行),回滚语义与 messageId 定位只有方向性描述。
- **F-12 GenUI 解冻条件**:"等待独立证明用户任务、注入边界、可访问性和审计成本"无量化标准,实质为无限期冻结。
- **F-15 语料"可验证性"分层机制**:如何识别 chunk 是否来自结构化来源(说明书/食物成分表/已审校数据)未述;"top-k 减半"的具体数值未给。
- **F-4 客户端契约校验**:表格列数、链接域白名单的校验规则与失败后的降级渲染行为未定义。
- **F-17/F-18"半年无消费方再归档"的时间基准**:从何时起算未明确。
- **F-8 设置页"当前会话受影响说明"**:调研原文用"考虑",非决定项。
- **工作量评估缺失**:调研全文无人日/工时量化,仅有 F-5"10 行内接线"与"前端约 30 文件/后端约 60 文件"的规模描述,排期只能依赖 P0/P1/P2 框架。
