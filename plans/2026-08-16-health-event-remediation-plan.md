...
- 依赖:无。中期增强「症状标签集合」(创建时勾选症状、与记录页症状库联动)为不确定点,本计划不排期。

#### 2. H-6 + H-9 历史事件详情接线与 reasonRecord 展示
...
#### 3. H-10 事件专属建议升级通知 + suggestion topic 补全
...
#### 5. C-1 `conditions` 进助手上下文 + 事件创建可选关联

- 现状:conditions 进 risk check LLM 上下文与就诊摘要,但无规则单独消费(半用)。
- 改造方案:保留为档案字段;助手 `get_user_profile` 等 read tool 可读疾病史以回答「结合我的情况」类问题;事件创建(`StartEventSheet` + `EventsService.create`)增加可选关联入口。**不进药物风险判断**(MVP 门控,见不确定点)。
...
- 改造方案:删除硬编码静态数据;`MineDashboard.alerts` 字段保留为真实数据出口,改造为真实「档案提醒」卡——接真实过敏/当前用药/档案缺口,由档案完整性计算驱动,成为 Mine 档案状态主卡组成部分。是否新建后端完整性接口见不确定点(现状完成度计算在前端 `lucent.dart`,可先复用前端口径)。
...
- 桌面/Web 形态挂起项统一引用 [`2026-08-14-product-surface-route.md`](2026-08-14-product-surface-route.md)(ADR-0012 待决策),不展开。

## 五、本计划内执行顺序

1. 改造项 2(H-6+H-9 详情接线):工程量小、闭环价值直接,且为后续 Review 改版前的可用性兜底。
2. 改造项 1(H-4 kind 筛选):纯 UI,与改造项 2 同区域可连续完成。
3. 改造项 3(H-10):依赖 today 计划的升级通知一节落地后接入。
4. 改造项 5(C-1 conditions):独立于其他改造,可并行。
5. 改造项 4(C-1 weightKg):依赖 record 计划 vital 基建,全局顺序第 6 位落地后启动。
6. 改造项 6(C-3 假卡真实化)与 7/8(unitSystem / onboardingCompleted):P2,随 C-1 字段决策统一收口,同步完成 C-4 完成度口径对齐。

## 六、不确定点(待决策)

- **R2 建议升级通知规则对齐**(H-10):规则定义与通知执行器细节在 today 计划,落地前需对齐信号口径与投递节流参数。
- **vital 时间序列基建空白**(C-1 weightKg):血糖/血压 vital 在调研文档中无现状描述,基建是新成立项,范围以 record 计划为准。
- **症状库/症状标签集合**(H-4 第二步):无方案细节、无排期,且依赖尚不存在的记录页症状库(当前记录页为自由文本)。
- **H-8 red flag 与事件药物对齐**:仅「后续可把事件关联用药传入风险检查」,无方案无排期。
- **H-7 标题更正端点**:以「真实用户反馈标题笔误高频」为触发条件,当前无决策。
- **C-1 `conditions` 进药物风险判断**:MVP 门控,待规则库成熟再议,刻意搁置。
- **H-1 选项预读失败重试**:仅「可加一行加载失败可重试」,是否纳入本期未定。
- **C-3 后端档案完整性计算**:改造方案要求「由后端档案完整性计算驱动」,但后端是否已有对应接口未交代(现状完成度计算在前端 `lucent.dart`);需决策新建后端接口还是复用前端口径。
- **C-3 优先级与严重性错配**:C-3 是全文唯一明确判定的假实现(硬编码假过敏),但优先级仅 P2,低于多个「真实现仅优化」的 P1;是否上调需计划评审确认。
