...
> 结论：**P0 已全部闭环**。**P1 跨页面共性 + 今日模块已完成**。**P2 打磨批已完成主体项**。剩余少量低级别打磨项。
...
> 仅列未闭环项；已 ✅ 的约 125 项不再复述。级别沿用原计划。

### 4.4 用药 / 搜索 / 扫码

- ❌（低）指标 chip 不可联动；频率切换静默清空星期；桌面预览空态一行小字；置信度无解释；线框 280×120 硬编码；结果头图无占位。
