...
## 职责与边界
- 管:`MineDashboard` 聚合读模型(`domain/entities/dashboard.dart`);档案编辑页与表单状态(`presentation/pages/`、`presentation/providers/health_edit_forms.dart`);同步失败横幅与同步失败页(`presentation/widgets/sections/sync_failed_*` + `presentation/pages/sync_failures.dart` + `presentation/providers/sync_failures.dart` + `presentation/mappers/sync_error_user_message.dart`)。
