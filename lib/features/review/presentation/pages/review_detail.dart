...
/// 单个历史事件的完整回顾详情页（`/review/review/:eventId`）。
///
...
class ReviewDetailPage extends ConsumerWidget {
  const ReviewDetailPage({super.key, required this.eventId});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PageScaffold(
      title: l10n.reviewReviewDetailTitle,
      child: detailAsync.when(
        // ReviewSkeletonView 是 Review 页骨架屏，内容高度超过一屏，需放在
        // 可滚动容器内复用（与 ReviewView 的骨架屏展示语义一致）。
        loading: () => const SingleChildScrollView(child: ReviewSkeletonView()),
