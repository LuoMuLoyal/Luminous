...
// 仍需要旧版周报的用户，按 d8c9c5f5e 旧 ReportPage 的 dashboard 装配重建
// 本兼容页（readiness 首卡 + 趋势/发现/建议历史 + AI 总结/规律 + 四张导出卡
// + 7/30 天范围切换的 ReportTopBar），推送式进入（`/report/legacy`）。
//
// 取舍说明：只重建移动端布局（与当前 ReviewView 一致，桌面宽度也渲染同一
// 布局），不复刻旧桌面端 DesktopTabShell 双栏外壳；导出/分享行为经共享的
// `handleReportExportAction` 与旧装配保持一致，不新增功能。
// ═══════════════════════════════════════════════════════════════════════════
...
class LegacyDashboardCompatPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PageStateSwitch<ReportDashboard>(
      state: pageState,
      ...
      fatalErrorBuilder: (error) => _buildShell(
        ...
        child: StateErrorView(
          title: AppLocalizations.of(context)!.reportErrorTitle,
          description: AppLocalizations.of(context)!.reportErrorDescription,
  ...
  Widget _buildReadyContent({
    ...
    final dashboardView = ReportDashboardView(
      ...
      onExportActionTap: (kind) => handleReportExportAction(context, ref, kind),
    );
