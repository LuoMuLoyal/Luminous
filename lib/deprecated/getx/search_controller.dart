...
@Deprecated('Use SearchNotifier (Riverpod) instead')
class SearchController extends GetxController {
  ...
  void commitSearch(BuildContext context, {String? emptyToast}) {
    ...
    if (normalized.isEmpty) {
      ToastUtils.instance.show(
        context,
        emptyToast ?? '请输入产品名称、批准文号或生产单位后再搜索',
      );
      return;
    }
    _lastError = null;
  ...
  Future<void> clearHistory(BuildContext context, {String? clearedToast}) async {
  ...
  Future<void> search({required bool reset}) async {
    ...
    if (reset) {
      ...
      _slowSearchHintTimer = Timer(const Duration(milliseconds: 300), () {
        if (isClosed || !_isActiveSearchRequest(requestId) || !_loading) return;
  ...
  void _syncDefaultRecentKeywords() {
    if (!_recentHistoryReady || _hasPersistedRecentKeywords) return;
    if (_localizedRecentDefaults.isEmpty) return;
    final defaults = List<String>.from(_localizedRecentDefaults);
    if (_recentKeywords.isEmpty) {
      if (_historyClearedByUser) return;
      _recentKeywords.addAll(defaults);
      _seededRecentKeywords = List<String>.from(defaults);
      return;
    }
    final seeded = _seededRecentKeywords;
    if (seeded == null || !_sameStringList(_recentKeywords, seeded)) return;
    if (_sameStringList(_recentKeywords, defaults)) return;
    _recentKeywords
      ..clear()
      ..addAll(defaults);
    _seededRecentKeywords = List<String>.from(defaults);
  ...
  void _syncDraftKeyword() {
    ...
    if (draftKeywordNotifier.value == next) return;
    draftKeywordNotifier.value = next;
  }

  void _handleScroll() {
    if (!scrollController.hasClients) return;
    if (_keyword.isEmpty || !_hasMore || _loadingMore || _loading) return;
