...
@Deprecated('Use DetailNotifier (Riverpod) instead')
class MedicineDetailController extends GetxController {
  ...
  Future<void> loadDetail() async {
    if (_loadingDetail || !_item.hasIdentity) return;
    ...
    try {
      ...
      if (isClosed || _isCanceledError(error)) return;
      _showError(error);
    } finally {
  ...
  Future<void> loadAiDetail({bool refresh = false}) async {
    if (_loadingAi || !_item.hasIdentity) return;
    ...
    try {
      ...
      if (_isLikelyNetworkFailure(error)) {
        _showError(
          error,
          fallback: _l10n?.medicineDetailAiNetworkErrorToast ?? '网络访问失败，请检查网络后重试',
        );
      } else {
        _showError(error);
      }
