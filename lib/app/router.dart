...
///
/// The five main tabs live inside a [StatefulShellRoute] whose custom
/// `navigatorContainerBuilder` ([ShellTabBranchContainer]) keeps the desktop
/// sidebar / mobile bottom navigation visible and cross-fades the branches when
/// switching between tab roots.
///
...
GoRouter appRouter(Ref ref) => GoRouter(
  ...
  routes: [
    StatefulShellRoute(
      ...
      // Tab roots use `NoTransitionPage` on purpose: the branch switch is
      // animated by `ShellTabBranchContainer` alone. A `CustomTransitionPage`
      // here was measured never to play on a tab switch (2026-09-12,
      // frame-by-frame probe) — the branch's first mount is added at full
      // opacity and a restored branch reuses its Navigator — so it was dead
      // configuration that still read as if it owned the tab transition.
      branches: [
        StatefulShellBranch(
          routes: [
            GoRoute(
