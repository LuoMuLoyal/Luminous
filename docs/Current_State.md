...
- Report now fetches the Lucent report dashboard on page entry for signed-in users, keeps section-level shimmer loading, shows explicit signed-out gating, and now wires the `给校医院 / For clinic` export card to the real Lucent data-export request flow for `hospital + pdf + last_7_days`.
...
- Additional report export kinds beyond `hospital + pdf + last_7_days`.
