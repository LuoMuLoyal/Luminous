...
- Dense settings-page UI is now being dragged over at both the page and core-wrapper levels: the top-level `SettingsPage` already uses page-local Forui tiles/switch rows directly, and the remaining shared settings wrappers in `core/widgets/settings/` now render through `FTile` / `FSwitch` instead of Material `InkWell` / `Switch`.
