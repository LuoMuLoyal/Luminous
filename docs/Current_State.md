...
- After the 2026-07-02 medicine/search phase, `rg -n "AppThemeSurface|AppTypographyTokens|AppSectionSurface|Icons\\." lib/features/medicine lib/features/search` returns no hits. The remaining old-theme removal work is now pushed outward to Record, shared common widgets, and the root bridge layer rather than the medicine/search feature surface.
...
- After the 2026-07-02 record finish pass, `rg -n "AppThemeSurface|AppTypographyTokens|AppSectionSurface|Icons\\." lib/features/record` returns no matches. The remaining migration debt is now outside `lib/features/record/`, mainly in shared common widgets and the root compatibility layer.
...
- Shared common widgets now mostly follow the same direction too: `app_state_views.dart`, `app_text_action.dart`, `app_status_pill.dart`, `app_image_placeholder.dart`, and `app_header_action_chip.dart` no longer read `AppThemeSurface` / `AppTypographyTokens`, and instead use direct Forui colors plus Material `textTheme`. In `lib/core/widgets/common/`, the remaining old wrapper debt is now essentially concentrated in `app_section_surface.dart`.
...
- The shared auth widget layer has now mostly crossed the same boundary too: `auth_branding.dart`, `auth_text_field.dart`, `auth_action_row.dart`, `auth_shell.dart`, `auth_status_message.dart`, and `auth_field_error.dart` no longer rely on `AppThemeSurface` / `AppTypographyTokens`, and `lib/features/auth/presentation/widgets/` now scans clean for `AppThemeSurface|AppTypographyTokens|AppSectionSurface|Icons.`. The remaining auth old-theme debt is now mainly in some page/section files rather than the shared widget primitives.
- The remaining feature-level bridge consumers have now also been pushed out of `lib/`: `account_settings_sections.dart`, `app_settings_master_toggle_page.dart`, `report_components.dart`, `medicine_safety_tip_style.dart`, and `medicine_reminder_formatters.dart` no longer require the old root bridge types. After the 2026-07-02 cleanup pass, `rg -n "AppThemeSurface|AppTypographyTokens|AppSectionSurface|Icons\\." lib` only returns the root bridge files themselves (`app_theme_extensions.dart`, `app_typography_tokens.dart`, `app_section_surface.dart`).
- The root bridge files have now also been reduced to compatibility-only status: `app_theme.dart` no longer injects `AppThemeSurface` into the runtime theme tree, `app_section_surface.dart` is just a thin Forui `FCard` header/content shell, and `app_theme_extensions.dart` / `app_typography_tokens.dart` are explicitly kept only for temporary compatibility and older tests during the final cleanup phase.
