...
- Shared child-page infrastructure is now partially Forui-based: child pages compose `FScaffold` / `FHeader` directly, `AppDialog` uses `FDialog.raw`, and `AppStateMessageView` / `AppStateErrorView` use `FCard` / `FButton`. Migrated pages still include a temporary transparent `Material` compatibility wrapper around content so that raw Material descendants such as `TextField` keep working until they are replaced with Forui form widgets.
...
- `AppColorTokens`, `AppSpacingTokens`, `AppRadiusTokens`, and `AppShadowTokens` still exist as compatibility names, but their actual values were aggressively reset on 2026-07-02 to track the current Forui-led foundation instead of preserving the old handcrafted palette/spacing/radius/type scale.
- `AppSpacingTokens` and `AppRadiusTokens` now also expose `level*` primary naming aligned with `AppShadowTokens` (`level1`, `level2`, ...). The legacy `xxs/xs/...` and `xs/sm/...` names still exist only as aliases while the rest of the runtime call sites are being moved over.
