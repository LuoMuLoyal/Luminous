...
- Root theming is now Forui-led and currently uses the official neutral preset: `lib/core/theme/app_theme.dart` derives the app's light/dark `ThemeData` from `FThemes.neutral` and injects `FTheme` at the app root instead of keeping the earlier green-skewed auth look.
...
- Shared child-page chrome is also on Forui primitives now: child pages compose `FScaffold`/`FHeader` directly, `AppBackButton` uses a Forui icon button, and shared state/dialog shells use `FCard`/`FButton`/`FDialog`.
- That bridge is now actively being collapsed rather than restyled in place: `settings_page.dart`, `notification_list_page.dart`, the rebuilt auth/account dialogs, the rewritten Today/Record interaction rows, and the medicine `FTappable` rows now render directly through `FScaffold` / `FHeader` / `FTappable` / `FDialog` without routing back through the deleted root wrapper files.
...
- The Today homepage top bar plus overview / priority / recommendation / todo sections now also render through direct Forui color/text semantics and Forui card shells. Those sections no longer rely on `AppSectionSurface`, `AppTypographyTokens`, or page-local `AppThemeSurface` reads, and the shared Today view-model icons feeding those surfaces have been switched from Material icons to Lucide.
- The Today AI summary shell has now followed too: `today_ai_summary_section.dart` and `today_page.dart` no longer keep the leftover `AppSectionSurface` / `AppThemeSurface` / `AppTypographyTokens` stack for the AI card and page background. After the 2026-07-02 cleanup pass, `rg -n "AppThemeSurface|AppTypographyTokens|AppSectionSurface|Icons\\." lib/features/today` returns no matches.
- The Mine homepage body now follows the same direction: `mine_dashboard_view.dart`, `mine_top_bar.dart`, `mine_account_hero.dart`, `mine_archive_section.dart`, `mine_status_overview.dart`, and the privacy-notice/shared-title helpers no longer pass `AppThemeSurface` / `AppTypographyTokens` through the page body. The visible Mine header/account/archive/status shells now render through direct Forui-aware color/text reads plus card-style layout, and the signed-out notice icon path was updated to Lucide.
- The Mine page shell and skeleton helpers have now followed too: `mine_page.dart`, `mine_components.dart`, `mine_skeleton_view.dart`, and `shell_deferred_content.dart` no longer keep feature-level `AppThemeSurface` reads for backgrounds, separators, or shared row/chip wrappers. After the 2026-07-02 cleanup pass, `rg -n "AppThemeSurface|AppTypographyTokens|AppSectionSurface|Icons\\." lib/features/mine lib/features/shell` returns no matches.
- The Report homepage body now follows the same direction too: `report_page.dart`, `report_dashboard_view.dart`, `report_top_bar.dart`, the score/metrics/trend/findings/patterns/export/AI-summary sections, and the reference notice no longer thread the old report-local `surface + typography + section surface` stack through the visible shell. The page now uses Forui-aware color/text reads plus card/button shells throughout the main dashboard surface, and report-local visible Material icons were replaced with Lucide equivalents. The remaining old theme bridge inside `lib/features/report/` is now limited to the shared `report_components.dart` sparkline helper.
- The medicine/search surface has now crossed the same boundary too: `medicine_page.dart`, `medicine_workspace_view.dart`, `medicine_mobile_dashboard_view.dart`, the mobile medicine sections, reminder detail/edit/form helpers, risk check page + risk tiles, and the search/add-precheck chain now read directly from `context.theme.colors` plus `Theme.of(context).textTheme`, use Forui card/group semantics, and no longer rely on `AppSectionSurface`, `AppThemeSurface`, or `AppTypographyTokens` in those feature surfaces.
- After the 2026-07-02 medicine/search phase, `rg -n "AppThemeSurface|AppTypographyTokens|AppSectionSurface|Icons\\." lib/features/medicine lib/features/search` returns no hits.
- Record is now partially across the same boundary in the most visible mobile-first panels: the NLP candidate/retry panels, meal-analysis summary/status widgets, image attachment card, meal-dish editor, date bar, mobile filter, AI input bar, quick-entry panel, mobile timeline, and new-entry panel now render through direct Forui-aware color/text reads plus Lucide icons rather than the old record-local `surface + typography + section surface` chain.
- After this record visible-panel pass, the remaining `record` old-theme debt is concentrated in larger shells such as `record_page.dart`, `record_detail.dart`, `record_dashboard_view.dart` desktop/top-level glue, and the sidebar/timeline/summary/shared section widgets rather than the most obvious mobile cards and entry panels.
- The record page/detail entry layer has now followed that same direction too: the top-level `record_page.dart` action chips/FAB/error state, `record_detail.dart` detail cards/image shell/edit-delete affordances, and the shared record header-chart divider helpers now use direct Forui-aware color/text reads plus Lucide icons instead of the older page-local `AppThemeSurface` / `AppTypographyTokens` presentation path.
- The record desktop dashboard sections have now mostly crossed the same boundary too: `record_dashboard_view.dart`, `record_sidebar.dart`, `record_summary_grid.dart`, and `record_timeline.dart` no longer thread `AppThemeSurface` / `AppTypographyTokens` through the desktop branch. Their visible shell now follows the official low-saturation Forui direction with white cards, thin neutral borders, black primary states, and Lucide icons instead of the older record-local section-surface path.
- The remaining record helper/dialog shells have now followed too: `record_quick_actions.dart`, `record_week_strip.dart`, `record_nlp_dialog.dart`, `record_voice_entry_dialog.dart`, and `record_ocr_entry_dialog.dart` no longer depend on `AppThemeSurface`, `AppTypographyTokens`, or `AppSectionSurface`, and their visible iconography now also uses Lucide instead of `Icons.*`.
...
- Shared dialog helper `showAppDialog` / `AppDialogShell` (`lib/core/widgets/common/app_dialog_shell.dart`) is the default for new dialogs; `RecordNlpDialog` and `MedicineAddPrecheckDialog` already use it.
...
- Auth feature tests have been fixed to match the Forui surface: deleted-widget tests were removed, `FilledButton` assertions became `FButton`, `TestForuiApp`/`TestAuthApp` replaced stale `AppTheme` bootstraps, and the register test now accepts the new `FCheckbox` terms-consent gate.
- The interaction-shell cleanup has now started following the same rule outside auth: `today_todo_section.dart`, `today_priority_section.dart`, `record_quick_actions.dart`, `record_date_bar.dart`, `record_nlp_dialog.dart`, `medicine_workspace_helpers.dart`, the mobile medicine sections/safety/reminder rows, `theme_settings_page.dart`, `ai_settings_page.dart`, `sleep_reminder_settings_page.dart`, `settings_page.dart`, and `notification_list_page.dart` no longer depend on `AppInkWell`, `AppDialog`, or `PageScaffoldShell` at their touched call sites.
- The login shell has been tightened further toward the official examples: the logo is now rendered without the old custom gradient/shadow container, and the password/code selector sits between the logo/title block and the form card instead of living inside the form itself.
- The auth migration now has its own reusable implementation note in `docs/Auth_Forui_Migration_Pattern.md`. That document captures the stable patterns we want to copy forward, and explicitly records that auth-only forced theme overrides and monochrome logo filters were attempted but rejected rather than kept as long-term solutions.
- The register form now also carries a real Forui `FCheckbox` consent gate for terms/privacy. Account creation stays disabled until the user accepts, and the old placeholder auth terms toast has been replaced by direct opens to the current public legal URLs (`/terms` and `/privacy` on `luminous.app`).
- `account_settings_page.dart` now also follows the official Forui sample direction more closely at the layout level: account management is split by a top `FTabs` selector, with one pane for overview/profile/email/identity management and one pane for password/delete-account actions, and each pane renders inside a single outer `FCard` instead of the old stack of nested cards.
- The remaining feature-level bridge consumers have now also been pushed out of `lib/`: `account_settings_sections.dart`, `app_settings_master_toggle_page.dart`, `report_components.dart`, `medicine_safety_tip_style.dart`, and `medicine_reminder_formatters.dart` no longer require the old root bridge types.
...
- **2026-07-03 follow-up (local surface replacement)**: Plan section 1.3 is now closed. The remaining hand-built chip/card/badge/avatar surfaces across `today_top_bar`, `medicine_page`, `mine_top_bar`, and the report sections (`report_metrics_grid`, `report_export_section`, `report_score_hero`, `report_findings_section`, `report_patterns_section`) were verified to already be implemented with native Forui widgets (`FButton.raw`, `FCard.raw`, `FBadge.raw`, `FAvatar.raw`). The active plan has been updated to delete the completed `1.3` and `Phase 2` sections; the next immediate action is Phase 1 icon cleanup.
- **2026-07-03 follow-up (Phase 1 icon cleanup)**: All Material3 `Icons.*` usages in runtime `lib/` (including data/domain repositories and entities) have been replaced with `FLucideIcons.*` equivalents. The remaining `Icons.*` references are only in generated `*.g.dart` / `*.freezed.dart` files. The active plan has been updated to delete the completed `Phase 1` section; next immediate action is Phase 3 wrapper inlining.
- **2026-07-03 follow-up (Phase 3 wrapper inlining)**: The thin visual-alias wrappers (`app_header_action_chip`, `app_icon_badge`, `app_image_placeholder`, `app_section_header`, `app_status_pill`, `app_text_action`, and the four `app_settings_*` rows/section) have been inlined into native Forui widgets and their files deleted. The remaining shared wrappers are those that encapsulate real behavior or layout: `app_back_button`, `app_dialog_shell`/`showAppDialog`, `app_divider`, `app_state_views`, and `app_toast`. The active plan has been updated to delete the completed `Phase 3` section; next immediate action is Phase 4 token finalization.
- **2026-07-03 follow-up (Phase 4 token alias cleanup)**: Verified that runtime `lib/` and `test/` no longer contain the legacy token aliases `AppThemeSurface`, `AppTypographyTokens`, `AppSectionSurface`, `AppColorTokens`, `AppShadowTokens`, or the legacy interaction aliases `AppInkWell` / `AppDialog`. Living docs (`AGENTS.md`, `Project_Guardrails.md`, `architecture.md`, and the active debt-paydown plan) were updated to stop presenting the deleted aliases as current design-system facts. `flutter analyze --no-pub` reports 0 issues.
- **2026-07-03 follow-up (Phase 4 layout-helper comments)**: Added file-level doc comments to `app_breakpoints.dart`, `app_layout_tokens.dart`, and `app_responsive_sizing.dart` explicitly identifying them as responsive layout helpers rather than visual design tokens. `flutter analyze --no-pub` reports 0 issues.
- **2026-07-03 follow-up (Phase 4 token finalization complete)**: `AppSpacingTokens` audited and unused `level11` removed; `AppRadiusTokens` confirmed mapped to Forui’s `FBorderRadius` scale; runtime `lib/` contains no Material `Icons.*` usages; all Phase 4 items in the active debt-paydown plan are complete. `flutter analyze --no-pub` reports 0 issues.
- **2026-07-03 follow-up (delete completed plan)**: `plans/2026-07-03-forui-debt-paydown-plan.md` deleted after all concrete action items were finished; remaining ongoing constraints (`forui_hooks` preference, no new `App*` wrappers) are enforced by `AGENTS.md` instead. `flutter analyze --no-pub` reports 0 issues.
