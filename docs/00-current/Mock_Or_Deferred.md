...
- `barcode_scanner_page_test` / `box_scan_test` 通过注入平台接口 fake 隔离真实设备能力：`PermissionHandlerPlatform.instance`（相机权限）、`MobileScannerPlatform.instance`（相机预览/条码流）、`ImagePickerPlatform.instance`（拍照），`paddleOcrProvider` 以 `AsyncValue.data(MockPaddleOcrEngine)` 覆盖（`paddleOcrProvider` 是 `FutureProvider`，override 时需包装在 `AsyncValue` 中），`ocrModelManagerProvider` 以 `AsyncValue.data(FakeOcrModelManager(modelsAvailable: true))` 覆盖（避免 OCR 流程因模型未下载而弹出下载对话框），`PaddleOcrNativePlatform.instance` 注入 `FakePaddleOcrNativePlatform`（`init` 签名包含 `required ModelPaths modelPaths`）。测试从不启动真实相机/相册/OCR 引擎。
- `paddle_ocr_provider_test`：`PaddleOcrEngine` 构造函数需要 `OcrModelManager` 参数，测试中传入 `FakeOcrModelManager`。`paddleOcrProvider` 是 `FutureProvider`，通过 `await container.read(paddleOcrProvider.future)` 获取引擎实例。新增模型缺失时抛出 `OcrModelsNotDownloadedException` 的测试用例。
