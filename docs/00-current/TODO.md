...
- 建立跨前后端 `Health Event` 合同：用户确认开始/结束，系统只能根据短期药物、提醒计划或连续症状提出建议，不能静默判定生病或康复
- 事件期症状输入收紧为开始时记录一次、此后每天最多一次“好转 / 差不多 / 加重”；详细症状、严重程度和备注保持可选
...
- 修正健康数据“自动同步”开关只有本地状态、没有同步协调器的误导；在真实同步链路完成前禁用或明确标成未启用
- 修复健康数据去重不能只使用 `kind|date|source`，避免同日同来源的多次饮水或睡眠片段被误删
...
- 健康数据导入的自动同步开关没有执行者，按日去重会丢同日多条记录；在已验证平台接入前不得宣传自动采集
