...
- 桌面端报告页现已与移动端对齐到同一回顾语义：
  - 顶部移除旧的 snapshot 状态块，只保留标题、时间范围和操作按钮。
  - 主内容首块改为 `readiness` 主卡，不再额外插入独立数据不足横幅。
- 移动端预览层只保留：
  - 报告预览评分
  - 健康趋势预览
  - 重点发现
- `历史建议回顾` 已补齐为真实数据区块：
  - 数据源来自通知接口中的 `ai_proactive_suggestion`
  - 当前展示最近 3 条建议通知，状态只区分 `已读 / 未读`
  - 当前不虚构 `已执行 / 已忽略 / 已延后`，等待 Lucent 提供一等行为反馈合同
