...
- Forui-first 编码统一性优化完成：Material 组件全面迁移、颜色/排版 token 化。语义颜色系统增量清理已完成 today/record/report/search/shell/medicine/assistant/mine/notification/core 全部批次，`colors.X.withValues(alpha: Y)` 直接 `FColors` 访问已迁移到 `SemanticColor` palette 预计算色调（`subtle`/`muted`/`border`/`solid`），范围外 alpha 值改为基于 `palette.solid(context).withValues(alpha:)`。
