...
- Today 根页已收口为 `主建议卡 → 次建议区 → 今日摘要 → 观察项 → 轻动作` 结构。建议引擎前端接入 Phase 1+2 已完成：API 客户端生成、Domain 实体层、Remote Data Source 就绪，`todaySuggestionProvider` 已接入 `TodayDashboardView`（ConsumerWidget），废弃的 `priorityItems` / `TodayPriorityItem` / `TodayPriorityItemType` 已彻底删除。
