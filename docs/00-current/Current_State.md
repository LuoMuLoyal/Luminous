...
- Sentry 生产错误上报已集成（2026-07-11）：保留 Talker 作为日志基础设施，新增 `sentry_flutter ^9.0.0`；通过自写 `SentryTalkerObserver`（`TalkerObserver` 子类）桥接，Talker 的 error/exception 事件自动转发 Sentry；`main.dart` 用 `runZonedGuarded` + `FlutterError.onError` 捕获未处理异常和 Framework 错误；`SENTRY_DSN` 通过 `--dart-define-from-file=.env` 注入，DSN 为空时 SDK 禁用；Talker 配置从 `enabled: !kReleaseMode` 改为 `enabled: true`（release 保留内存历史 + Sentry 转发，控制台输出仍仅 debug）；`run_guarded.dart` 从 `talker.error()` 改为 `talker.handle()` 以传递完整异常和堆栈。
