...
## 当前实现状态：停止扩展，未来产品方向未决

- 现有桌面适配和代码保留，本文件继续记录已经实现的事实。
- 现有 Flutter 桌面端不是当前首发与验证表面，不继续发行、宣传或追求与手机端功能对等。
- 后续手机端产品方向变化不自动要求桌面端同步；只有共享代码回归或明确批准的维护任务可以触及桌面专项实现。
- 停止扩展不是永久放弃桌面产品。当前大屏假设是阅读、比较和理解手机端难以看清的纵向健康信息；Next.js Web + Tauri 2 是待研究候选，不要求复用现有 Flutter 桌面形态。
