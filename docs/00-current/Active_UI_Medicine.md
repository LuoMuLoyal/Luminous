...
- 编辑/启停保存走整组 upsert（`PUT /api/v1/user/medicine-reminders/group`，单事务），组内缺失槽服务端软删，不再逐槽半保存。客户端经生成 API `medicineRemindersControllerUpsertGroupV1` 提交（domain 输入映射为 `UpsertMedicineReminderGroupDto`，空串可选字段省略）。
