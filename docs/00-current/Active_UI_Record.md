...
- 阶段 8 详情页增强：
  ...
  - 单位制换算（改造项 7）：water 进度卡文案按档案 `unitSystem` 展示——imperial 时总量与目标均换算 fl oz（各自保留 1 位小数，`recordDetailDailyWaterProgressOz`，如 "18.6 / 67.6 fl oz"），metric/未设置显示 ml；进度条 `value` 始终用原始 ml 比例（仅文案单位换算）。单位制读取 `healthContextSnapshotProvider`（loading/error 回退公制），仅 water 记录且有 ml 合计时 watch。摘要瓦片、饮水角标、today 概览等其它 ml 展示点**不做**（保持存储口径 ml）。
