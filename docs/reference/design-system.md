...
- Auth 页与其它子页同一套骨架（2026-09-13 删除 `AuthShell`/`MobileAuthShell`/`DesktopAuthShell`/`AuthFormPanel` 整条壳层）：`PageScaffold` → `SingleChildScrollView` → `ResponsiveContentFrame` → `Padding(vertical: width < Breakpoints.mobile ? Spacing.xl2 : Spacing.xl3)` → 内容。不再有白色卡片面板、不再有 `Center + ConstrainedBox(maxWidth: 560)` 的窄列约束，内容直接铺在页面级背景上（登录/注册/忘记密码的品牌 logo 与副标题作为内容首项保留）。
...
- 多字段底部 sheet 用共享外壳 `SheetSurface` + `SheetDragHandle`（`lib/core/widgets/common/dialog/sheet_drag_handle.dart`）：Forui 的 `showFSheet` 只给布局/手势/遮罩、**不画背景**，不套外壳会把内容直接画在变暗的页面上；`SheetSurface` 负责 `colors.card` + 圆角顶边并让底边贴到视口外。配 `useSafeArea: true` 时**内部不要再套 `SafeArea`**——Forui 只补上/左/右（`SafeArea(bottom: false)`），再套一层会把内容连同背景抬离底边、露出下面的暗色页面，看起来像「sheet 与底部 tab 栏之间的间隙」。参考实现：`lib/features/record/presentation/widgets/dialogs/sleep_quick_entry_sheet.dart`、`lib/features/record/presentation/widgets/dialogs/water_quick_entry_sheet.dart`。
- 需要「往上拖展开」的 sheet：`showFSheet(mainAxisMaxRatio: null)` + body 内 `DraggableScrollableSheet(expand: false, initialChildSize/minChildSize/maxChildSize, snap: true)`；builder 拿到的 controller **必须**交给内部可滚动组件（否则手势不联动），并按 Forui 注释补 `ScrollConfiguration(dragDevices: {touch, mouse, trackpad})` 以支持桌面/Web 拖动。`mainAxisMaxRatio: null` 只是解除高度上限，单靠它拖不上去；也不要把「内容本身就矮」当成不用展开的理由——没有可展开余量时上拖是空操作，用户会读成「拖不动」。反过来，**内容本体就是长列表的 sheet 不必包** `DraggableScrollableSheet`：`mainAxisMaxRatio: null` 已让它长到整屏、内部滚动即可（`scan/presentation/pages/barcode_scanner.dart` 的两处扫码 sheet）。字段较多时**不要**用 `FSelect` 的弹出层承载选项（空间不足会被裁到只露前几行且拖不动），改用内联 pill chip 行——共享组件 `PillChip`（`lib/core/widgets/common/control/pill_chip.dart`）就是这套配方，record 的睡眠质量与症状严重度/目录行已接入（review 的范围 chip 仍是页面级私有实现，视觉同源）。
...
- auth 页面（login/register/forgot-password/change-email/account-settings）直接组合 `FButton` / `FTabs` / `FToast` / `FDialog`，页面骨架与其它子页一致（原 `auth_shell.dart` 壳层已删除，见上）。
