...
- 需要「往上拖展开」的 sheet：`showFSheet(mainAxisMaxRatio: null)` + body 内 `DraggableScrollableSheet(expand: false, initialChildSize/minChildSize/maxChildSize, snap: true)`；builder 拿到的 controller **必须**交给内部可滚动组件（否则手势不联动），并按 Forui 注释补 `ScrollConfiguration(dragDevices: {touch, mouse, trackpad})` 以支持桌面/Web 拖动。`mainAxisMaxRatio: null` 只是解除高度上限，单靠它拖不上去；也不要把「内容本身就矮」当成不用展开的理由——没有可展开余量时上拖是空操作，用户会读成「拖不动」。反过来，**内容本体就是长列表的 sheet 不必包** `DraggableScrollableSheet`：`mainAxisMaxRatio: null` 已让它长到整屏、内部滚动即可（`scan/presentation/pages/barcode_scanner.dart` 的两处扫码 sheet）。字段较多时**不要**用 `FSelect` 的弹出层承载选项（空间不足会被裁到只露前几行且拖不动），改用内联 pill chip 行——配方见 `lib/features/review/presentation/widgets/sections/ai_summary.dart` 的 `_RangeChip`。
