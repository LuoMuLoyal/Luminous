...
- New dialogs must use `showAppDialog` / `AppDialogShell` (`lib/core/widgets/common/app_dialog_shell.dart`) instead of hand-writing the `Dialog` + `ConstrainedBox` + `SafeArea` + `Padding` + scroll wrapper.
