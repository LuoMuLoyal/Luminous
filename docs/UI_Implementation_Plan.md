...
- Today, Record, Medicine, Report, and Mine share the same partial-loading pattern: page-level providers can stay pending while the mobile dashboard structure renders immediately through `AppSkeletonScope` and skeleton slots for backend-backed values.
