...
| 文件 | 格式 | 是否适合喂给 AI |
|---|---|---|
| `forui-docs/llms-full.txt` | 纯 Markdown，聚合了全部官方文档和代码示例 | ✅ **首选** |
| `forui-docs/llms.txt` | 索引，含每页标题、描述和链接 | ✅ 快速查范围 |
| `forui-docs/docs/content/docs/**/*.mdx` | MDX（Markdown + JSX），引用了缺失的 `snippets/*.json` | ❌ JSX 噪音大、示例不完整 |
| Pub/Cache 源码 | Dart 源码 | ✅ 查 API 签名和默认样式时最准确 |
...
- **根主题**：`lib/app/app.dart` 从 `FThemes.neutral.light.touch` / `.dark.touch` 派生 `ThemeData`，再用 `FTheme` 包裹整棵树。
- **页面 header**：
...
- 设置页分组标签：用 `AppTypographyToken.level4.body(context).copyWith(color: colors.mutedForeground, fontWeight: FontWeight.w600)`。
...
- 项目封装：`lib/core/widgets/common/app_dialog_shell.dart` 的 `showAppDialog` / `AppDialogShell`，统一处理 `maxWidth`、padding、滚动、键盘 inset。
...
- 不要再保留 `AppThemeSurface`、`AppTypographyTokens`、`AppSectionSurface`、`AppColorTokens`、`AppShadowTokens`、`AppInkWell`、`AppDialog` 等已删除别名。
