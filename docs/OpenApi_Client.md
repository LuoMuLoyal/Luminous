...
- WeChat Web OAuth login uses generated `OAuthAuthorizeDataDto`, `OAuthAuthorizeResponseDto`, and `OAuthCallbackDto` through `AuthRemoteDataSource`. OAuth-only Lucent users may have `email: null`, so `AuthUser.email` is nullable and UI should fall back to nickname, ID, or signed-out copy where appropriate.
- Re-generated on 2026-06-05 after Lucent added WeChat Web/Mobile OAuth endpoints. Lucent export currently reports **30 paths / 80 schemas**. The generated package now includes `OAuthAuthorizeDataDto`, `OAuthAuthorizeResponseDto`, `OAuthCallbackDto`, and `OAuthCodeCallbackDto`.
