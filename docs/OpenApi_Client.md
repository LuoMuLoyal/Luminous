...
- `AuthUser` stores `emailVerifiedAt` and derives `emailVerified` locally. Account detail responses also map `hasPassword`, `lastLoginAt`, and linked OAuth identities into the auth session user.
