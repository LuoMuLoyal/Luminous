...
- WeChat Web OAuth login uses generated `OAuthAuthorizeDto`, `OAuthAuthorizeDataDto`, `OAuthAuthorizeResponseDto`, and `OAuthCallbackDto` through `AuthRemoteDataSource`. Desktop login passes a loopback `callbackUri` to Lucent so the browser callback can redirect back to the app. OAuth-only Lucent users may have `email: null`, so `AuthUser.email` is nullable and UI should fall back to nickname, ID, or signed-out copy where appropriate.
