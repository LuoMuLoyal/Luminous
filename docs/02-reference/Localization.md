...
Last updated: 2026-08-20 (P1-2 B1 新增通知文案：`notificationTypeAiWeeklyInsight`、`sleepReminderNotificationBody`、`sleepReminderNotificationChannelName`、`sleepReminderNotificationChannelDescription`，均在 notification 分片；既有 Mine/Record/Report 文案保持不变。)
...
## 2026-08-18 P1-1 摘要网格饮水 ml 单位键

- 新增 `recordSummaryMlUnit`（zh: ml / en: ml）到 `lib/l10n/src/record_zh.arb` / `record_en.arb`（record 分片）。
- `RecordCopyKey` 新增 `summaryMlUnit`（summary 键组末尾），`recordCopy` 补 `summaryMlUnit => l10n.recordSummaryMlUnit`；`LucentRecordRepository._toDaySummary` 的 water 摘要项在存在可累加 ml 记录时使用该键作为单位。
- 所有权在 record 分片（`record*` 前缀）；经 `dart scripts/arb_tools.dart merge` + `flutter gen-l10n` 合并进 `app_*.arb`（生成文件为 gitignore 产物）。
- 顶部 `Last updated` 已同步记录本次新增。

## 2026-08-18 H-6 + H-9 历史事件详情页与触发记录文案键

- 新增 `reportReviewDetailTitle`（zh: 事件回顾 / en: Event review）到 `lib/l10n/src/report_zh.arb` / `report_en.arb`（report 分片）：`/report/review/:eventId` 详情页 `PageScaffold` 标题。
...
## 2026-08-18 改造项 7 unitSystem 单位制显示切换键

- 新增 `mineProfileWeightLb`（zh: {weight}lb / en: {weight}lb，int 占位符 `weight`）到 `lib/l10n/src/mine_zh.arb` / `mine_en.arb`（mine 分片）：档案区体重行 imperial 单位制下的 lb 展示（`archive.dart` `_profileMeta` 经 `weightInLb` 换算后 round()），格式对齐既有 `mineProfileWeightKg`。
- 新增 `recordDetailDailyWaterProgressOz`（zh/en: {total} / {target} fl oz，String 占位符 `total`/`target`）到 `lib/l10n/src/record_zh.arb` / `record_en.arb`（record 分片）：详情页饮水进度卡 imperial 单位制下的总量/目标 fl oz 展示（`detail.dart` 传 `waterInFlOz(ml).toStringAsFixed(1)` 的已格式化字符串，进度条 value 仍用 ml 比例）。
- 换算工具 `lib/features/health_context/domain/services/unit_conversion.dart` 为纯 Dart 展示换算，不引入新 l10n 键。
- 所有权分别在 mine / record 分片；经 `dart scripts/arb_tools.dart merge` + `flutter gen-l10n` 合并进 `app_*.arb`（生成文件为 gitignore 产物）。
- 顶部 `Last updated` 已同步记录本次新增。

## 2026-08-18 改造项 6 C-3 Mine「档案提醒」真实化键

- 新增 4 键到 `lib/l10n/src/mine_zh.arb` / `mine_en.arb`（mine 分片，均为 int 占位符 `count`）：
  - `mineAlertAllergyCount`（zh: {count} 项 / en: {count} items）：过敏卡右侧 badge，格式对齐既有「2 项」风格。
  - `mineAlertAllergyMore`（zh: 等 {count} 项 / en: and {count} in total）：过敏卡副标题超出前 2 个 label 时追加的「等 N 项」后缀。
  - `mineAlertMedicineCount`（zh: {count} 种 / en: {count} medicines）：用药卡右侧 badge。
  - `mineAlertMedicineMore`（zh: 等 {count} 种 / en: and {count} in total）：用药卡副标题超出前 2 个 displayName 时追加的「等 N 种」后缀。
