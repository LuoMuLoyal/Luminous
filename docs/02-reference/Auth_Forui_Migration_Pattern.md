...
This document records the stable migration pattern used to move the auth surface from the old
custom widget stack to direct Forui composition. It is meant to be reused by later page migrations.
...
- Do not bury a top-level mode choice deep inside the form content when it changes the entire form
   branch.
...
- Avoid nested `card inside card inside section` stacks unless the inner card represents a clearly
   distinct object.
...
- If the asset color is wrong, fix the asset or the shared theme, not by stacking more local visual
   effects by default.
...
`flutter test` was intentionally skipped during this migration phase per the active acceleration
directive.
