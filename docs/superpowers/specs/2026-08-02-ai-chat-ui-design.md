...
- 将当前 Material `Drawer` 替换为页面内 Forui 风格的不透明 push drawer，从左侧推入并把聊天页整体向右移动。
...
- 不改变独立 AI 设置页中的 AI 开关、记忆和健康上下文开关。
...
- 顶部栏从左到右固定为返回、历史对话、新会话、设置；历史入口打开页面内 push drawer，设置入口跳转 `/settings/ai`。
...
- 使用页面内 `Stack`、固定宽度和 Forui surface，从左侧推入；drawer 背景不透明，聊天页整体向右移动，不使用 modal barrier。
- 抽屉顶部提供搜索框、新建会话按钮和关闭按钮。
- 搜索按照会话标题过滤当前已加载的 `recentConversations`；空搜索显示完整列表。
- 搜索结果仍按 today / this week / older 分组，空结果显示 Forui 状态视图。
- 点击会话先关闭 drawer，再复用现有 `openConversation`；新建会话先关闭 drawer，再复用现有 `clearConversation`。
- 搜索文本是抽屉本地 UI 状态，不写入 controller，不触发网络请求。

## Component boundaries

- `AssistantPage`: 继续持有输入/滚动 controller，并负责 drawer 的开关、推移动画和会话回调协调。
- `AssistantPageBody`: 负责页面状态分支、顶部动作和主内容布局。
- `AssistantStatusBar`: 缩减或迁移为欢迎区所需的非技术状态提示；设置入口跳转现有独立 AI 设置页。
- `AssistantConversationSurface`: 继续组合消息列表、错误提示和输入区。
- `AssistantInputBar`: 只负责输入区视觉和输入交互，不承载会话业务逻辑。
- `AssistantConversationDrawer`: 改为页面内不透明 Forui drawer 内容壳，保持回调契约。
...
- 添加/更新 assistant widget tests，覆盖：四入口 header、push drawer 位移、空会话欢迎内容、单层输入表面、搜索过滤、分组会话点击和关闭 drawer、设置页跳转。
