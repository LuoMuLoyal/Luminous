...
- `ProfileSettingsPage`：`ConsumerStatefulWidget` + `ProfileSettingsNotifier`（Notifier），管理 loading/saving/deleting/gender + load/save/delete/avatar。旧 `ProfileSettingsController` → `deprecated/getx/`。
