...
/// File system paths to the PaddleOCR ONNX model files.
///
/// The ONNX model files are **not** bundled in the APK. The host app must
/// download them to the device's file system (e.g. the app's internal
/// storage directory) and provide the paths here when calling [PaddleOcr.init].
class ModelPaths {
  /// Absolute file system path to the detection model (inference.onnx).
  final String detModelPath;

  /// Absolute file system path to the recognition model (inference.onnx).
  final String recModelPath;

  /// Absolute file system path to the recognition config (inference.yml).
  /// This file ships with the plugin as a Flutter asset and is copied to
  /// the file system by the host app alongside the ONNX models.
  final String recConfigPath;

  const ModelPaths({
    required this.detModelPath,
    required this.recModelPath,
    required this.recConfigPath,
  });

  Map<String, dynamic> toMap() => {
    'detModelPath': detModelPath,
    'recModelPath': recModelPath,
    'recConfigPath': recConfigPath,
  };
}
