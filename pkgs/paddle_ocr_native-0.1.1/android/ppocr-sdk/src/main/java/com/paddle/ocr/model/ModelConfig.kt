...
data class ModelConfig(
    ...
    companion object {
        fun parse(context: Context, assetPath: String): ModelConfig {
            ...
            return parseFromContent(content, assetPath)
        }

        fun parseFromFile(filePath: String): ModelConfig {
            val content = try {
                File(filePath).readText()
            } catch (t: Throwable) {
                throw OCRError.ConfigParseFailed(filePath, t)
            }
            return parseFromContent(content, filePath)
        }

        private fun parseFromContent(content: String, sourcePath: String): ModelConfig {
            val characterDict = try {
                extractCharacterDict(content, sourcePath)
            } catch (e: OCRError.ConfigParseFailed) {
                throw e
            } catch (t: Throwable) {
                throw OCRError.ConfigParseFailed(sourcePath, t)
            }
