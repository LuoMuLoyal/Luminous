...
class OCREngine private constructor(
        ...
        val configured = try {
            when (modelSource) {
                is ModelSource.Asset -> {
                    ortManager.loadModels(modelSource.detAsset, modelSource.recAsset)
                    ModelConfig.parse(context, modelSource.recConfigAsset)
                }
                is ModelSource.File -> {
                    ortManager.loadModelsFromFiles(modelSource.detPath, modelSource.recPath)
                    ModelConfig.parseFromFile(modelSource.recConfigPath)
                }
            }
        } catch (t: Throwable) {
            ortManager.release()
    ...
    companion object {
        operator fun invoke(
            ...
            context, config, engineConfig,
            ModelSource.Asset(detModelAsset, recModelAsset, recConfigAsset),
        )
        ...
        fun fromFiles(
            ...
            detModelPath: String,
            recModelPath: String,
            recConfigPath: String,
        ): OCREngine = OCREngine(
            context, config, engineConfig,
            ModelSource.File(detModelPath, recModelPath, recConfigPath),
        )
    }
