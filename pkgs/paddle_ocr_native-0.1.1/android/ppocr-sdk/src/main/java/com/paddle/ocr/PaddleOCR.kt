    ...
    companion object {
        ...
        suspend fun create(context: Context): PaddleOCR {
            ...
            return withContext(Dispatchers.IO) {
                val engine = OCREngine.invoke(appContext, PaddleOCRConfig(), EngineConfig())
                PaddleOCR(engine)
            }
        }
        ...
        suspend fun create(
            ...
            val appContext = context.applicationContext
            return withContext(Dispatchers.IO) {
                val engine = OCREngine.invoke(appContext, config, engineConfig)
                PaddleOCR(engine)
            }
        }
        ...
        /// Creates a PaddleOCR instance from file system paths.
        ///
        /// Model files are read from the device's file system (not Android assets).
        /// The host app is responsible for downloading the model files before calling.
        suspend fun createFromFile(
            ...
            val appContext = context.applicationContext
            return withContext(Dispatchers.IO) {
                val engine = OCREngine.fromFiles(
                    appContext, config, engineConfig,
