...
class ORTSessionManager(
    ...
    fun loadModelsFromFiles(detFilePath: String, recFilePath: String) {
        val loadStart = System.currentTimeMillis()
        env = OrtEnvironment.getEnvironment()
        val opts = OrtSession.SessionOptions().apply {
            setOptimizationLevel(OrtSession.SessionOptions.OptLevel.ALL_OPT)
        ...
        try {
            val ortEnv = env ?: throw OCRError.ModelLoadFailed("OCR", Exception("Environment not initialized"))
            val detBytes = readModelFile(detFilePath)
            val recBytes = readModelFile(recFilePath)
            try {
                detSession = ortEnv.createSession(detBytes, opts)
            } catch (t: Throwable) {
                throw OCRError.ModelLoadFailed("detection", t)
    ...
    private fun readModelFile(filePath: String): ByteArray {
        return try {
            java.io.File(filePath).readBytes()
        } catch (t: Throwable) {
            throw OCRError.ModelNotFound(filePath, t)
        }
    }
