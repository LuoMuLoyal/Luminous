...
class PaddleOcrNativePlugin : FlutterPlugin, MethodCallHandler {
    ...
    private fun handleInit(call: MethodCall, result: Result) {
        ...
        val modelPathsMap = call.argument<Map<String, Any>>("modelPaths")
        val config = buildConfig(configMap)
        val engine = EngineConfig(
            numThreads = (engineMap["numThreads"] as? Number)?.toInt() ?: 4,
        )

        // Model files are downloaded by the host app to the device file system.
        // The Dart side provides absolute file paths via modelPaths.
        val detModelPath = modelPathsMap?.get("detModelPath") as? String
        val recModelPath = modelPathsMap?.get("recModelPath") as? String
        val recConfigPath = modelPathsMap?.get("recConfigPath") as? String

        if (detModelPath.isNullOrEmpty() || recModelPath.isNullOrEmpty() ||
            recConfigPath.isNullOrEmpty()) {
            result.error("INVALID_ARGS", "modelPaths (detModelPath, recModelPath, recConfigPath) are required", null)
            return
        }

        scope.launch {
            try {
                Log.i(TAG, "init: config=$config, engine=$engine, det=$detModelPath")
                ...
                val ocr = PaddleOCR.createFromFile(
                    context = context,
                    config = config,
                    engineConfig = engine,
